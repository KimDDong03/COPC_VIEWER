...
- Example-only `Stream on camera move` behavior that renders from the currently loaded hierarchy, queues one background camera-targeted hierarchy prefetch at a time, and avoids rebuilding the full node dropdown during the stream update.
...
- Camera streaming is prototype-oriented; it prefetches a small number of hierarchy pages in the background and now applies configurable render-point budgets. The smoothness benchmark now covers multiple bundled sample paths, but the recommended defaults still need calibration against larger external COPC samples and repeated frame-time measurements.
...
3. Tune hierarchy prefetch depth and cache policy with byte-aware limits and camera-priority hints.
