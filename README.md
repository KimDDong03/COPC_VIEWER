...
`npm run qc` runs the main release-quality checks sequentially: tests, license/SBOM evidence, build, the three-backend renderer benchmark, contest/cold/warm camera-stream smoothness QC, package smoke, browser example smoke, local-file browser smoke, and `git diff --check`. `npm run qc:contest-device` runs the same release chain without duplicating the one-session warm check, then performs the stricter three-fresh-session median regression gate. Keep these checks sequential because the browser smoke commands rebuild the same `dist/example` output directory.
`npm run qc:contest-device` adds the committed same-GPU performance comparison after the release checks. The current approved baseline targets the recorded RTX 3060 WebGL renderer and deliberately rejects incomparable adapters.
The same remote URL and local-file browser rendering smoke runs in the GitHub
Actions workflow `Example Browser Smoke` for pushes, pull requests, and manual
dispatches.
